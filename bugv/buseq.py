...
"""
seq_ids, id2seq = FastaSeqsTool().fasta2dict(filename)
...
class PosFileParser():
    ...
    def __init__(self, pos_file_class_name, file_name="", data=""):
        self._parser = pos_file_class_name(file_name, data)
...
class FastaSeqsTool():
    ...
    def translate(self, seqs, print_all=0):
        translate_func = FastaSeqTool().translate
        retain_flag = 1
        for seqid, seq in seqs:
            seq = translate_func(seq)
            if print_all:
                yield([seqid, seq, retain_flag])
            else:
                yield([seqid, seq])

def seq_id2pos_data(seq_ids):
    return (["", [[i]]] for i in seq_ids)
...
class FastaSeqs():
    ...
    def iter_seqs(self, bydict=0, split_id=0, split_func=0, filters=[]):
        '''
    ...
    def extract_seq_by_pos(self, pos_file="", fasta_file="", pos_data=None, pos_type="bu_pos_file", split_id=0, split_func=0):
        """
        pos_data: a list. [[seq_id]] or [[seq_id, new_name]] or [[seq_id, start, end, strand]]
        """
        pos_class = self._pos_type2class[pos_type]
        pos_data = PosFileParser(pos_class, pos_file, pos_data).parse()
        if fasta_file:
    ...
    def extract_seq_by_pos2file(self, pos_file, fasta_file, fileout, line_word_lim = 100, split=0, pos_data=None):
        self._tool.write_fasta(self.extract_seq_by_pos(pos_file, fasta_file, split_id=split, pos_data=pos_data), fileout, line_word_lim)
    
    def extract_merge_seq_by_pos2file(self, pos_file, fasta_file, fileout, line_word_lim = 100, split=0, pos_data=None):
        self._tool.write_fasta(self.extract_and_merge_seq_by_pos(pos_file, fasta_file, split_id=split, pos_data=pos_data), fileout, line_word_lim)
    ...
    def write_seq_2_txt(self, fileout):
        '''
        Iter self data.
        '''
        seqs = self.iter_seqs()
...
class TranslateFastaSeqs(FastaSeqs):
    
    def _reset(self):
        self._file = ""
        self._filters = ["upper", "rna2dna", "translate"]
        self.seqids = []
        self.fastas = {}
...
class BsmapMethratio():
    ...
    def load_region_data(self, region, bin_length):
        pass
...
def iter_pos_file(filein, parse_pos_func = None, header = False):
    ...
    def _parse_bed(l):
        ...
        #2020.10.18 modification
        #Change 1-based mode to 0-based.
        #start, end = int(start), int(end)
...
def write_seq_2_txt(filefasta, fileout):
    fastas = FastaSeqs(filefasta)
    fastas.write_seq_2_txt(fileout)
...
def polyA_finder(seq="", base="A",
    ...
    max_window_num =  math.ceil(len(seq)/window_length)
    window_counts = []
...
def translate2file(filefasta, fileout, line_max_word=0):
    fastas = TranslateFastaSeqs(filefasta)
    fastas.load_file(filefasta)
    fastas.write_fasta(fileout, line_max_word)
